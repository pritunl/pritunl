...
# [SYNCHRONOUS] Called after a user has authenticated with Duo when loging
# into the web console. Must return True or False to accept auth request and
# an organization name or None. If an organization name is included the user
# will be added to that organization. If Duo is used as a secondary
# authentication method the organization name from Duo if set will have
# priority over the organization name from the primary SSO provider.
...
# [SYNCHRONOUS] Called after a user has authenticated with Duo when connecting
# to a VPN server. Must return True or False to accept auth request.
